...
#:
#:  坐标直接取主页面 `play_btn` 模板的中心:
#:    `config\templates.json` 里 `play_btn.region` = (2,95,115,105)
#:    -> 程序取整后的中心 **(59,147)**。这是主页面左上角进入卡牌选择页的按钮,
#:    与卡组选择页右下角的「开始」按钮保持足够距离。
#:  ★ 为什么不用原来的右下安全点:它靠近卡组选择页的 `deck_ok_btn`,
#:    补点时可能误触发「开始」进入对局。
#:  ★ 抖动 ±25/±15 与 `click_center` 一致 —— 固定一个像素点容易正好落在
#:    "没反应的那一格"上,抖动留出余量(但不越界,见下面的 min/max 夹取)。
POST_MATCH_POINT = (59, 147)
...
#:  这个点是主页面 `play_btn` 的中心,以后要换点只改这一个常量
#:  (别在别处再写死坐标)。
